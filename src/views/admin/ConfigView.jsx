...
export const ConfigView = ({ businessId }) => {
  ...
  useEffect(() => {
    ...
    loading,
    config.deliveryConfig?.coordenadasLocal,
    config.deliveryConfig?.direccionLocal,
  ]);
    ...
    try {
      ...
      if (config.deliveryConfig) {
        if (config.deliveryConfig.modo === "porKm") {
          const { coordenadasLocal, rangos, ...rest } = config.deliveryConfig;
          payloadPublico.deliveryConfig = {
            ...rest,
            modo: "porKm",
            kmMaximo: Number(config.deliveryConfig.kmMaximo) || 0,
            precioBaseDelivery:
              Number(config.deliveryConfig.precioBaseDelivery) || 0,
            precioPorKm: Number(config.deliveryConfig.precioPorKm) || 0,
            direccionLocal: config.deliveryConfig.direccionLocal || "",
            direccionVerificada:
              config.deliveryConfig.direccionVerificada || "",
          };
          if (coordenadasLocal) {
            payloadPublico.deliveryConfig.coordenadasLocal = coordenadasLocal;
          }
        } else {
      ...
      if (config.deliveryConfig?.coordenadasLocal) {
        payloadPrivado.coordenadasLocal =
          config.deliveryConfig.coordenadasLocal;
      }
  ...
  const handleVerificarUbicacion = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setDeliveryConfig("coordenadasLocal", {
          lat: result.lat,
          lng: result.lng,
        });
                  ...
                  >
                    <div className="pago-header">
                      <label className="pago-toggle">
                        <input
                          ...
                          onChange={(e) =>
                            setPago(key, "activo", e.target.checked)
                          }
                        />
                        <span>
                          {icon} {label}
                    ...
                    {metodo.activo && (
                      <div className="pago-entrega-opts">
                        <span className="pago-entrega-label">
                          Disponible en:
                        </span>
                        {retiroActivo && (
              ...
              <section className="config-section">
                ...
                <div className="delivery-modo-selector">
                  ...
                  >
                    ...
                    <span className="modo-desc">
                      Define rangos con mapa visual
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="modo-desc">
                      Precio automático según distancia
                    </span>
                  </button>
                </div>
                ...
                {(deliveryConfig?.modo || "rangos") === "rangos" ? (
                  <>
                    <p className="config-hint">
                      Configura la dirección del local y rangos de km con su
                      precio. El costo se calcula según la distancia al cliente
                      (Google Maps).
                    </p>
                      ...
                      {geocodeStatus &&
                        ...
                        !geocodeLoading && (
                          <p className="delivery-geocode-error">
                            No se encontró la dirección. Intenta con más
                            detalle.
                          </p>
                        )}
                    </div>
                  ...
                  <div className="delivery-porkm-form">
                    ...
                    <div className="config-row">
                      ...
                      />
                      <p className="config-hint">
                        Monto fijo que se cobra independiente de la distancia.
                        Puede ser $0.
                      </p>
                    </div>
                    ...
                    {Number(deliveryConfig.precioPorKm) > 0 && (
                      <div className="delivery-porkm-preview">
                        ...
                        {[1, 2, 3, 5, 8]
                          .filter(
                            (km) =>
                              km <= (Number(deliveryConfig.kmMaximo) || 99),
                          )
                          .map((km) => (
                            <div key={km} className="preview-row">
                              ...
                              <span>
                                $
                                {(
                                  (Number(deliveryConfig.precioBaseDelivery) ||
                                    0) +
                        ...
                        <p className="preview-formula">
                          Fórmula: $
                          {Number(deliveryConfig.precioBaseDelivery) || 0} base
                          + km × ${Number(deliveryConfig.precioPorKm) || 0}
                        </p>
                      </div>
                    )}
                      ...
                      {h.abierto ? (
                        <div className="horario-times">
                          ...
                          <input
                            ...
                            value={h.fin}
                            onChange={(e) =>
                              setHorario(dia, "fin", e.target.value)
                            }
                          />
